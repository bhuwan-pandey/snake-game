...
class Snake():
    def __init__(self) -> None:
        self.width = 10
        self.position_of_head = utility.pygame.Vector2(
            utility.RESUMED_SCREEN.get_width()/2, (utility.RESUMED_SCREEN.get_height()/2))
        self.position_of_directions = [utility.pygame.Vector2(
            (utility.RESUMED_SCREEN.get_width()/2)+10, utility.RESUMED_SCREEN.get_height()/2)]
        self.current_direction = utility.pygame.K_LEFT
    ...
    def move(self):
        if self.current_direction == utility.pygame.K_UP:
            self.move_up()
        elif self.current_direction == utility.pygame.K_DOWN:
            self.move_down()
        elif self.current_direction == utility.pygame.K_LEFT:
            self.move_left()
        elif self.current_direction == utility.pygame.K_RIGHT:
            self.move_right()

    def move_up(self):
        # prevent reversing the direction in a straight line
        if self.current_direction != utility.pygame.K_DOWN:
            # donot/no-need insert/bend if the moving direction and pressed direction are same
            if self.current_direction and self.current_direction != utility.pygame.K_UP:
                self.position_of_directions.insert(0, utility.pygame.Vector2(
                    self.position_of_head.x, self.position_of_head.y))
                self.directions_of_position.insert(
                    0, {'direction': utility.pygame.K_UP, 'draw_next': True})
            self.current_direction = utility.pygame.K_UP
            self.position_of_head.y -= utility.MOVE_FACTOR
            self.slither()
    ...
    def move_down(self):
        ...
        if self.current_direction != utility.pygame.K_UP:
            # donot/no-need insert/bend if the moving direction and pressed direction are same
            if self.current_direction and self.current_direction != utility.pygame.K_DOWN:
                self.position_of_directions.insert(0, utility.pygame.Vector2(
                    self.position_of_head.x, self.position_of_head.y))
                self.directions_of_position.insert(
                    0, {'direction': utility.pygame.K_DOWN, 'draw_next': True})
            self.current_direction = utility.pygame.K_DOWN
            self.position_of_head.y += utility.MOVE_FACTOR
            self.slither()

    def move_left(self):
        # prevent reversing the direction in a straight line
        if self.current_direction != utility.pygame.K_RIGHT:
            # donot/no-need insert/bend if the moving direction and pressed direction are same
            if self.current_direction and self.current_direction != utility.pygame.K_LEFT:
                self.position_of_directions.insert(0, utility.pygame.Vector2(
                    self.position_of_head.x, self.position_of_head.y))
                self.directions_of_position.insert(
                    0, {'direction': utility.pygame.K_LEFT, 'draw_next': True})
            self.current_direction = utility.pygame.K_LEFT
            self.position_of_head.x -= utility.MOVE_FACTOR
            self.slither()

    def move_right(self):
        # prevent reversing the direction in a straight line
        if self.current_direction != utility.pygame.K_LEFT:
            # donot/no-need insert/bend if the moving direction and pressed direction are same
            if self.current_direction and self.current_direction != utility.pygame.K_RIGHT:
                self.position_of_directions.insert(0, utility.pygame.Vector2(
                    self.position_of_head.x, self.position_of_head.y))
                self.directions_of_position.insert(
                    0, {'direction': utility.pygame.K_RIGHT, 'draw_next': True})
            self.current_direction = utility.pygame.K_RIGHT
            self.position_of_head.x += utility.MOVE_FACTOR
            self.slither()
    ...
    def slither(self, appear=True,phase_shift=True,search_for_food=True,prevent_self_injury=True):
        ...
        if self.directions_of_position[-1]['direction'] == utility.pygame.K_UP:
            self.position_of_directions[-1].y -= utility.MOVE_FACTOR
        ...
        elif self.directions_of_position[-1]['direction'] == utility.pygame.K_DOWN:
            self.position_of_directions[-1].y += utility.MOVE_FACTOR
            if len(self.position_of_directions) > 1:
                if self.position_of_directions[-1].y >= self.position_of_directions[-2].y:
                    self.position_of_directions.pop()
                    self.directions_of_position.pop()
        elif self.directions_of_position[-1]['direction'] == utility.pygame.K_LEFT:
            self.position_of_directions[-1].x -= utility.MOVE_FACTOR
            if len(self.position_of_directions) > 1:
                if self.position_of_directions[-1].x <= self.position_of_directions[-2].x:
                    self.position_of_directions.pop()
                    self.directions_of_position.pop()
        elif self.directions_of_position[-1]['direction'] == utility.pygame.K_RIGHT:
            self.position_of_directions[-1].x += utility.MOVE_FACTOR
            if len(self.position_of_directions) > 1:
                if self.position_of_directions[-1].x >= self.position_of_directions[-2].x:
                    self.position_of_directions.pop()
                    self.directions_of_position.pop()
        if not utility.ALLOW_THROUGH_WALL:
            if self.position_of_head.x < 5 or self.position_of_head.x+5 >= utility.RESUMED_SCREEN.get_width() or self.position_of_head.y < 5 or self.position_of_head.y+5 >= utility.RESUMED_SCREEN.get_height():
                utility.game_is_paused = True
        if appear:
            self.appear()
        if phase_shift:
            self.phase_shift()
    ...
    def appear(self):
        ...
        for index_of_current_position_of_direction, current_position_of_direction in enumerate(reversed_position_of_directions):
            if len(reversed_position_of_directions) > index_of_current_position_of_direction+1 and reversed_directions_of_position[index_of_current_position_of_direction]['draw_next']:
                utility.pygame.draw.line(utility.RESUMED_SCREEN, "red", (current_position_of_direction.x, current_position_of_direction.y), (
        ...
        utility.pygame.draw.line(utility.RESUMED_SCREEN, "red", (self.position_of_directions[0].x, self.position_of_directions[0].y), (
            self.position_of_head.x, self.position_of_head.y), self.width)

        # draw head/curve and the eyes
        if self.current_direction == utility.pygame.K_UP:
            # head
            utility.pygame.draw.circle(
                utility.RESUMED_SCREEN, 'red', (self.position_of_head.x+1, self.position_of_head.y), 5)
            # eyes
            utility.pygame.draw.circle(
                utility.RESUMED_SCREEN, 'blue', (self.position_of_head.x-2, self.position_of_head.y), 1)
            utility.pygame.draw.circle(
                utility.RESUMED_SCREEN, 'blue', (self.position_of_head.x+3, self.position_of_head.y), 1)
            # tongue
            utility.pygame.draw.line(utility.RESUMED_SCREEN, 'red', (self.position_of_head.x,
            ...
            # head
            utility.pygame.draw.circle(
                utility.RESUMED_SCREEN, 'red', (self.position_of_head.x, self.position_of_head.y+1), 5)
            # eyes
            utility.pygame.draw.circle(
                utility.RESUMED_SCREEN, 'blue', (self.position_of_head.x, self.position_of_head.y-2), 1)
            utility.pygame.draw.circle(
                utility.RESUMED_SCREEN, 'blue', (self.position_of_head.x, self.position_of_head.y+3), 1)
            # tongue
            utility.pygame.draw.line(utility.RESUMED_SCREEN, 'red', (self.position_of_head.x,
    ...
    def get_horizontal_body_coordinates(self):
        ...
        for index_of_position, position_of_direction in enumerate(reversed_position_of_directions):
            ...
            if reversed_directions_of_position[index_of_position]['direction'] in (utility.pygame.K_LEFT, utility.pygame.K_RIGHT):
    ...
    def get_vertical_body_coordinates(self):
        ...
        for index_of_position, position_of_direction in enumerate(reversed_position_of_directions):
            # if the position/line starts towards either right or left
            if reversed_directions_of_position[index_of_position]['direction'] in (utility.pygame.K_UP, utility.pygame.K_DOWN):
                # length 1 means only tail exist. so return range from tail to head
                if len(reversed_position_of_directions) == 1 or index_of_position+1 >= len(reversed_position_of_directions):
                    coordinates.append(
                        [position_of_direction, self.position_of_head])
    ...
    def prevent_self_injury(self):
        ...
        for index_of_position, position_of_direction in enumerate(reversed_position_of_directions):
            ...
            if index_of_position+1 >= len(reversed_position_of_directions):
                continue
            if self.current_direction == utility.pygame.K_UP:  # snake moving up
                # body line moving towards left
                if reversed_directions_of_position[index_of_position]['direction'] == utility.pygame.K_LEFT:
                ...
                elif reversed_directions_of_position[index_of_position]['direction'] == utility.pygame.K_RIGHT:
                ...
                elif reversed_directions_of_position[index_of_position]['direction'] == utility.pygame.K_UP:
                ...
                # body line moving towards down
                elif reversed_directions_of_position[index_of_position]['direction'] == utility.pygame.K_DOWN:
                    if self.position_of_head.x > position_of_direction.x-self.width/2 and self.position_of_head.x < position_of_direction.x+self.width/2 and self.position_of_head.y < reversed_position_of_directions[index_of_position+1].y and self.position_of_head.y > position_of_direction.y:
                        utility.game_is_paused = True
                        print("up down")
            elif self.current_direction == utility.pygame.K_DOWN:  # snake moving down
                # body line moving towards left
                if reversed_directions_of_position[index_of_position]['direction'] == utility.pygame.K_LEFT:
                    if self.position_of_head.x > reversed_position_of_directions[index_of_position+1].x and self.position_of_head.x < position_of_direction.x and self.position_of_head.y > position_of_direction.y-self.width/2 and self.position_of_head.y < position_of_direction.y+self.width/2:
                        utility.game_is_paused = True
                ...
                # body line moving towards down
                    ...
                    if self.position_of_head.x > position_of_direction.x-self.width/2 and self.position_of_head.x < position_of_direction.x+self.width/2 and self.position_of_head.y > position_of_direction.y and self.position_of_head.y < reversed_position_of_directions[index_of_position+1].y:
                        utility.game_is_paused = True
                        print("down down")
            elif self.current_direction == utility.pygame.K_LEFT:  # snake moving down
                # body line moving towards left
                if reversed_directions_of_position[index_of_position]['direction'] == utility.pygame.K_LEFT:
                    if self.position_of_head.x < position_of_direction.x and self.position_of_head.x > reversed_position_of_directions[index_of_position+1].x and self.position_of_head.y > position_of_direction.y-self.width/2 and self.position_of_head.y < position_of_direction.y+self.width/2:
                        utility.game_is_paused = True
                ...
                # body line moving towards down
                    ...
                    if self.position_of_head.x < position_of_direction.x+self.width/2 and self.position_of_head.x > position_of_direction.x-self.width/2 and self.position_of_head.y > position_of_direction.y and self.position_of_head.y < reversed_position_of_directions[index_of_position+1].y:
                        utility.game_is_paused = True
                        print("left down")
            elif self.current_direction == utility.pygame.K_RIGHT:  # snake moving down
                # body line moving towards left
                if reversed_directions_of_position[index_of_position]['direction'] == utility.pygame.K_LEFT:
                    if self.position_of_head.x > reversed_position_of_directions[index_of_position+1].x and self.position_of_head.x < position_of_direction.x and self.position_of_head.y > position_of_direction.y-self.width/2 and self.position_of_head.y < position_of_direction.y+self.width/2:
                        utility.game_is_paused = True
            ...
            if food.position.x > 0 and food.position.y > 0:  # x/y less than 1 means food not available/rendered
                if self.position_of_head.x > food.position.x-food.size and self.position_of_head.x < food.position.x+food.size and self.position_of_head.y > food.position.y-food.size and self.position_of_head.y < food.position.y+food.size:
                    ...
                    if self.current_direction == utility.pygame.K_UP:
                        ...
                        if self.position_of_head.y+(food.calorie/2) < utility.RESUMED_SCREEN.get_height():
                            self.position_of_head.y += food.calorie/2
                    elif self.current_direction == utility.pygame.K_LEFT:
                        if self.position_of_head.x-(food.calorie/2) > 0:
                            self.position_of_head.x -= food.calorie/2
                    elif self.current_direction == utility.pygame.K_RIGHT:
                        if self.position_of_head.x+(food.calorie/2) < utility.RESUMED_SCREEN.get_width():
                            self.position_of_head.x += food.calorie/2
                    # increase tail by half the calorie
                    if self.directions_of_position[-1]['direction'] == utility.pygame.K_UP:
                        if self.position_of_directions[-1].y+(food.calorie/2) < utility.RESUMED_SCREEN.get_height():
                            self.position_of_directions[-1].y += food.calorie/2
                    elif self.directions_of_position[-1]['direction'] == utility.pygame.K_DOWN:
                        if self.position_of_directions[-1].y-(food.calorie/2) > 0:
                            self.position_of_directions[-1].y -= food.calorie/2
                    elif self.directions_of_position[-1]['direction'] == utility.pygame.K_LEFT:
                        if self.position_of_directions[-1].x+(food.calorie/2) < utility.RESUMED_SCREEN.get_width():
    ...
    def total_body_length(self):
        ...
        for index_of_current_position_of_direction, current_position_of_direction in enumerate(reversed_position_of_directions):
            if len(reversed_position_of_directions) > index_of_current_position_of_direction+1 and reversed_directions_of_position[index_of_current_position_of_direction]['draw_next']:
                if reversed_directions_of_position[index_of_current_position_of_direction]['direction'] in (utility.pygame.K_LEFT, utility.pygame.K_RIGHT):
        ...
        # count to head
        if self.current_direction in (utility.pygame.K_LEFT, utility.pygame.K_RIGHT):
                ...
                if self.current_direction == utility.pygame.K_LEFT:
                    if self.position_of_head.x < 1:
                        self.position_of_directions.insert(
                            0, utility.pygame.Vector2(0, self.position_of_head.y))
                        self.directions_of_position.insert(
                            0, {'direction': self.current_direction, 'draw_next': False})
                        self.position_of_directions.insert(0, utility.pygame.Vector2(
                            utility.RESUMED_SCREEN.get_width(), self.position_of_head.y))
                        self.directions_of_position.insert(
                            0, {'direction': self.current_direction, 'draw_next': True})
                        self.position_of_head.x = self.position_of_head.x+utility.RESUMED_SCREEN.get_width()
                        # reduce tail without recursing
                        self.slither(False,False,False,False)
                        self.phase_shifted = True
                elif self.current_direction == utility.pygame.K_RIGHT:
                    if self.position_of_head.x >=utility.RESUMED_SCREEN.get_width():
                        self.position_of_directions.insert(
                            0, utility.pygame.Vector2(utility.RESUMED_SCREEN.get_width(), self.position_of_head.y))
                        self.directions_of_position.insert(
                            0, {'direction': self.current_direction, 'draw_next': False})
                        self.position_of_directions.insert(0, utility.pygame.Vector2(
                        ...
                        self.slither(False,False,False,False)
                        self.phase_shifted = True
                elif self.current_direction == utility.pygame.K_UP:
                    if self.position_of_head.y <1:
                        self.position_of_directions.insert(
                            0, utility.pygame.Vector2(self.position_of_head.x,0))
                        self.directions_of_position.insert(
                            0, {'direction': self.current_direction, 'draw_next': False})
                        self.position_of_directions.insert(0, utility.pygame.Vector2(
                            self.position_of_head.x,utility.RESUMED_SCREEN.get_height()))
                        self.directions_of_position.insert(
                            0, {'direction': self.current_direction, 'draw_next': True})
                        self.position_of_head.y = self.position_of_head.y+utility.RESUMED_SCREEN.get_height()
                        # reduce tail without recursing
                        self.slither(False,False,False,False)
                        self.phase_shifted = True
                elif self.current_direction == utility.pygame.K_DOWN:
                    if self.position_of_head.y >=utility.RESUMED_SCREEN.get_height():
                        self.position_of_directions.insert(
                            0, utility.pygame.Vector2(self.position_of_head.x,utility.RESUMED_SCREEN.get_height()))
                        self.directions_of_position.insert(
                            0, {'direction': self.current_direction, 'draw_next': False})
                        self.position_of_directions.insert(0, utility.pygame.Vector2(
